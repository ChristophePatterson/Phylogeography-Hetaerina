...
# Write out lines
writeLines(c(length(RAD.loci), "", do.call("c", g_phocs)), con = paste0(dir.path, "G-Phocs/", SNP.library.name, "_N", N_loci, ".gphocs"))
...
# Write out pop file
write.table(assignFile, file = paste0(dir.path,"/G-Phocs/GPhocs_samples_N",N_loci,".txt"), col.names=F, row.names = F, quote=F)
