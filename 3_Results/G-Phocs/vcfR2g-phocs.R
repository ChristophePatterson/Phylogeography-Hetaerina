...
# SNP library
SNP.library.filter <- ".all.snps.NOGTDP10.MEANGTDP10_200.Q60.SAMP0.8"
...
current.contig <- vcf.SNPs.N@fix[1,1]
...
for(i in 2:contig.dimensions[1]){
  # Checks is difference between this and the previous contig is less than the set loci distance
...
summary(loci.stats)
summary(loci.stats$loci.called.bp/loci.stats$loci.length)
...
# Removes loci that have less than min.loci.length called
