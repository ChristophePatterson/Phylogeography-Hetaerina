...
# # # # # # ## # # # # # ## # # # # # #
##### ALL runs from all plots ####
# # # # # # ## # # # # # ## # # # # # #
### Produce plots that use hetero specific SNPs for g-phocs
input.dir.top <- "4_Manuscript/data/G-Phocs/"
N_select <- 2000
model.names <- paste0(c("americana_dg/G-Phocs-a1-b20-titia-Iso-N","titia_dg/G-Phocs-a1-b20-americana-Iso-N",
                        "americana_dg/G-Phocs-a1-b20-titia-migR-N","titia_dg/G-Phocs-a1-b20-americana-migR-N"),
                      N_select) 
model.names <- c(model.names, c("denovo/G-Phocs-a1-b20-titia-Iso-N3","denovo/G-Phocs-a1-b20-americana-Iso-N3",
                                "denovo/G-Phocs-a1-b20-titia-migR-N3","denovo/G-Phocs-a1-b20-americana-migR-N3"))

model.names <- c(model.names, paste0(c("titia_dg/G-Phocs-a1-b20-titia-Iso-N","americana_dg/G-Phocs-a1-b20-americana-Iso-N",
                                       "titia_dg/G-Phocs-a1-b20-titia-migR-N","americana_dg/G-Phocs-a1-b20-americana-migR-N"),
                                     N_select))
any(duplicated(model.names))
...
theta.plots <- ggplot(theta.trace) +
  geom_histogram(aes(theta_ind, fill = run), col = NA, alpha = 0.5, binwidth = 0.02) +
  geom_histogram(aes(theta_ind, col = run), fill = NA, binwidth = 0.02, show.legend = F) +
  facet_wrap(~Pop.read, ncol = 2, scales = "free_y") +
  #scale_fill_manual(values = cbPalette[1:8]) +
  xlim(c(0, max(theta.trace$theta_ind))) +
  labs(x = "Effective Population size (Millions)", y = "Count",
...
unique(mig.trace$mig.dir)
mig.dir.names <- list(
  "m_amer.N..amer.S"=expression(paste(italic("H. americana"), " North into ", italic("H. americana"), " South")),
  "m_amer.S..amer.N"=expression(paste(italic("H. americana"), " South into ", italic("H. americana"), " North")),
  "m_titia.NAtl..titia.SAtl"=expression(paste(italic("H. titia"), " North into ", italic("H. titia"), " South")),
  "m_titia.SAtl..titia.NAtl"=expression(paste(italic("H. titia"), " South into ", italic("H. titia"), " North"))
  )
mig.dir.labeller <- function(variable,value){
  return(mig.dir.names[value])
}

mig.plots <- ggplot(mig.trace) +
  geom_histogram(aes(mig.rate.prop, fill = run), col = NA, alpha = 0.5, binwidth = 0.000001) +
  geom_histogram(aes(mig.rate.prop, col = run), fill = NA, binwidth = 0.000001, show.legend = F) +
  # geom_histogram(aes(mig.rate.prop, fill = mig.dir)) +
  facet_wrap(~mig.dir, labeller = mig.dir.labeller) +
  theme_bw() +
  labs(x = "Migration rate (Percentage of effective population size)", y = "Count",
       fill = "Run") +
  theme(legend.position = "bottom") +
...
trace.df
write.table(trace.df, paste0("4_Manuscript/data/G-Phocs/","G-Phocs_results_all_", gsub(u, replacement = "-", pattern ="\\."),".txt"),
            row.names = F, quote=F)
write.csv(trace.df, paste0("4_Manuscript/data/G-Phocs/","G-Phocs_results_all_", gsub(u, replacement = "-", pattern ="\\."),".csv"),
          row.names = F, quote=F)
