...
r <- ggplot() +
  ...
  geom_point(data = hybrid.sites[!duplicated(hybrid.sites$Lat),], 
  ...
  geom_label_repel(data = hybrid.sites[!duplicated(hybrid.sites$Lat),], 
                   ...
                   min.segment.length = 0, show.legend = F, seed = 12345, nudge_y = hybrid.sites$nudge.dir[!duplicated(hybrid.sites$Lat)],
                   nudge_x = -0.2, col = "black") +
  scale_fill_manual(values = het.cols[c(1,3)]) +
  coord_sf(xlim = c(-94, -95.5), ylim = c(16, 17.5)) +
  xlab("Longitude") +
  ylab("Latitude")
