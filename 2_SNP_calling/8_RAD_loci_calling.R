...
top.cov.samples <- lapply(unique(pop_assign$pop), function(x) {
    ...
    if(dim(pop_assign_temp)[1]>max.samples){
        return((pop_assign_temp[order(pop_assign_temp$cov, decreasing = T),])[1:max.samples,])
    }
    if(dim(pop_assign_temp)[1]<=max.samples){
...
writeLines(con = paste0(dir.path,SNP.library.name,"_PHYLIP_subsample.txt"), text = pop_assign_N$sample)